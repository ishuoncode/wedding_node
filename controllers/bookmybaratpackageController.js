...
exports.createPackage = async (req, res) => {
    try {
      ...
      if (Array.isArray(req.body)) {
        ...
        packages = await BookMyBaratPackage.insertMany(req.body, { ordered: false });
      } else {
      ...
      if (error.code === 11000) {
        ...
        res.status(400).json({ message: "Duplicate package name(s) found. Some packages may not have been created." });
      } else {
...
exports.deletePackage = async (req, res) => {
  try {
    const deletedPackage = await BookMyBaratPackage.findByIdAndDelete(req.params.id);
    if (!deletedPackage) {
      return res.status(404).json({ message: "Package not found" });
    }
